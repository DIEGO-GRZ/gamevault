...
router.get('/', async (req, res) => {
  try {
    ...
    // Enriquecer cada item con datos de IGDB (name, image),
    // ya que la tabla library solo guarda el igdb_id.
    const enriched = await Promise.all(
      result.rows.map(async (item) => {
        try {
          const game = await igdb.getGameById(item.igdb_id);
          return {
            ...item,
          ...
          return { ...item, name: 'Juego desconocido', image: null };
        }
      })
    );
